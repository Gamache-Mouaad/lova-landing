      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="  p-3.5 grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {plans.map((plan, idx) => (
            ...
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-[#FFD54F] text-[#2D2D2D] text-1xl font-bold px-3 py-1 rounded-[7px] whitespace-nowrap">
                  الأفضل قيمة
                </div>
              )}
              ...
              <ul className="  space-y-3 mb-8 flex-1 text-right">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-2 text-[#4A4A55]">
                    ...
                    <span className="text-[19px] font-light md:text-[19px]">{feature}</span>
                  </li>
                ))}
              </ul>
