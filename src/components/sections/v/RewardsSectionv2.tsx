...
const cardBgWhite = '#FFFFFF';
const cardBorder = '#F0F0F0';

// Reward data with brand‑aligned colors (earned badges use brand purple/gold)
const rewards = [
  {
...
function BadgeCard({ reward, index }: BadgeCardProps) {
  ...
  return (
    ...
    >
      {/* Shimmer effect for earned on hover */}
      {isEarned && (
        <motion.div
      ...
      {/* Status corner (checkmark or lock) */}
      <div
...
export const RewardsSection = () => {
  ...
  return (
    <section dir="rtl" ref={ref} className="relative bg-white py-20 px-4 md:px-8 overflow-hidden">
      ...
      {/* Decorative blobs (very light) */}
      <div className="absolute top-0 left-0 w-64 h-64 rounded-full bg-[#F1EEFF] blur-3xl opacity-40 -translate-x-1/2 -translate-y-1/2" />
      <div className="absolute bottom-0 right-0 w-80 h-80 rounded-full bg-[#FFF3D6] blur-3xl opacity-40 translate-x-1/3 translate-y-1/3" />
